...
LOCAL_VGGISH_WEIGHTS = Path(__file__).with_name('vggish-pretrained.pth')
...
def vggish(feature_extract: bool) -> Tuple[VGGish, int]:
    ...
    if LOCAL_VGGISH_WEIGHTS.is_file():
        state_dict = torch.load(LOCAL_VGGISH_WEIGHTS, map_location='cpu')
    else:
        state_dict = hub.load_state_dict_from_url(VGGISH_WEIGHTS, map_location='cpu')
